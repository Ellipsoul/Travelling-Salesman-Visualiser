...
export class GridcommService { //Use of a service enables multiple-way communication between ANY components as opposed to child->parent or parent->child.
  ...
  changeSelPointMessage(newselpoints: {x: number; y:number}[]){ //function to completely modify the 'message' - useful for emptying the array
  ...
  addToSelPointsMessage(inpoint: {x: number; y:number}){ //function to append to the 'message' - useful for adding objects to the array
  ...
  removeFromSelPointsMessage(inpoint: {x: number; y:number}){ //function to remove from the 'message' - useful for removing objects from the array
